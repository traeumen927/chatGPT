...
이 앱은 사용자가 질문과 함께 **이미지**나 실시간 사진 촬영 결과를 첨부할 수 있도록 구성되어 있습니다.
첨부된 이미지는 GPT API 요청 시 함께 전송되어 분석 및 답변에 활용됩니다.

📸 **이미지는 다음 흐름을 보여줍니다:**

1. 하단의 ➕ 버튼을 눌러 사진 촬영 또는 앨범을 선택할 수 있는 메뉴를 여는 화면
...
> 이미지는 OpenAI의 Vision API로 전송되며, 첨부는 UI 상에서 직관적으로 구성되어 있습니다.
...
모든 기록의 `updatedAt` 필드는 클라이언트 시간이 아닌 서버 시간을 이용해 저장되어,
기기마다 시간이 달라도 동일한 순서로 정렬됩니다.
...
Firestore의 `conversations` 컬렉션과 사용자 프로필 정보가 저장되는 `profiles` 경로는 인증된 사용자만 접근할 수 있도록 설정해야 합니다.
